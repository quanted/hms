...
namespace HMSWebServices.Controllers
{
    public class WSPrecipitationController : ApiController
    {
        ...
        public string Get(string param)
        {
            ...
            {
                data = result.GetPrecipitationData(out errorMsg, parameters[0], parameters[1], parameters[2], parameters[3], parameters[4], Convert.ToBoolean(parameters[5]));
            }
            else if (param.Contains("ShapeFile"))       //Location provided by shapefile.
            {
                ...
                data = result.GetPrecipitationData(out errorMsg, parameters[1], parameters[2], parameters[3], Convert.ToBoolean(parameters[4]), shapefilePath);
            }  
            if (errorMsg.Contains("Error")) { DeleteTempShapefiles(); return errorMsg; }
        ...
        private string[] ParseParameters(out string errorMsg, string param)
        {
            ...
            if (result.Length == 0) { errorMsg = "Error: invalid number of parameters."; return null; }
            return result;
        }
