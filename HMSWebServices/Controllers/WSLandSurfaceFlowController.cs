...
namespace HMSWebServices.Controllers
{
    ...
    {
        /// <summary>
        /// Getes land surface flow data using the parameters provided in the param string.
        /// </summary>
        ...
        public string Get(string param)
        {
            ...
            else if (parameters.ContainsKey("shapefilePath"))       //Location provided by shapefile.
            {
                ...
                data = result.GetLandSurfaceFlowData(out errorMsg, parameters["startDate"], parameters["endDate"], parameters["source"], Convert.ToBoolean(parameters["localTime"]), unzippedShapefile);
        ...
        public string Post([FromBody]LandSurfaceFlow param)
        {
            string data = "";
            string errorMsg = "";
            if (errorMsg.Contains("Error")) { return errorMsg; }
            ...
            {
                data = result.GetLandSurfaceFlowData(out errorMsg, param.latitude, param.longitude, param.startDate, param.endDate, param.source, Convert.ToBoolean(param.localTime));
            }
            else
            {
                ...
                data = result.GetLandSurfaceFlowData(out errorMsg, param.startDate.ToString(), param.endDate.ToString(), param.source, Convert.ToBoolean(param.localTime), unzippedShapefile);
