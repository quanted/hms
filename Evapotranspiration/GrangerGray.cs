...
namespace Evapotranspiration
{
    public class GrangerGray
    {
        ...
        public void GrangerGrayMethod(double tmin, double tmax, double tmean, int jday, double shmin, double shmax,
                                      double wind, double solarRad, out double relHumidityMin, out double relHumidityMax,
                                      out double petGG, out string errorMsg)
        {
            ...
            double PR = 101.3;  // Pressure in kilo pascals
            ...
            double ALBEDO = albedo;  // Albedo is input by the user.
            ...
            try
            {
                ...
                RS = solarRad;   // Units are in MJ/(m^2 day). Conversion from W/m^2 to MicroJ/(m^2 day) was done in NLDAS2.

                //  RS = (0.25 + 0.5 * (SS*NN)) * RA;
                ...
                WIND = (2.626 + 1.381 * u) * (es - ea);
                part1 = WIND / (WIND + Rnet / latent); // PAR4 in the VB code
                GG = 1 / (0.793 + 0.2 * Math.Exp(4.902 * part1)) + 0.006 * part1;

                PET = (Rnet / latent) * (slope * GG / (slope * GG + gama)) + ((gama * GG * WIND) / (slope * GG + gama));  // mm/day
        ...
        public ITimeSeriesOutput getNldasData(out string errorMsg, ITimeSeriesInput inpt)
        {
            ...
            foreach (KeyValuePair<string, List<string>> timeseries in nldasTempOutput.Data)
            {
                ...
                timeseries.Value[8] = petGG.ToString("F4", CultureInfo.InvariantCulture);
                timeseries.Value.RemoveAt(9);
            }
            nldasTempOutput.Dataset = "Evapotranspiration";
            nldasTempOutput.DataSource = "grangergray";
        ...
        public ITimeSeriesOutput getGldasData(out string errorMsg, ITimeSeriesInput inpt)
        {
            ...
            gldasTempOutput.DataSource = "grangergray";
