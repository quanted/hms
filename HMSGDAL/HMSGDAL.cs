    ...
    public class HMSGDAL
    {
        ...
        public void CellAreaInShapefileByGrid(out string errorMsg, double[] coordinate, double cellWidth)
        {
            ...
            try
            {
                ...
                int aX = (Int32)Math.Ceiling((Math.Abs(transformedCoordinate[0] - geom.EnvelopeInternal.MinX) / cW));                           //Cell x value of upperleft corner of grid from centroid
                int aY = (Int32)Math.Ceiling((Math.Abs(geom.EnvelopeInternal.MaxY - transformedCoordinate[1]) / cW));                           //Cell y value of upperleft corner of grid from centroid          
                int iMax = (Int32)Math.Ceiling((geom.EnvelopeInternal.MaxX - geom.EnvelopeInternal.MinX) / cW);                                 //Total data grid cells along x
                int jMax = (Int32)Math.Ceiling((geom.EnvelopeInternal.MaxY - geom.EnvelopeInternal.MinY) / cW);                                 //Total data grid cells along y
