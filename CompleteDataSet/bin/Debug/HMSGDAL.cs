...
namespace HMSGDAL
{
    public class HMSGDAL
    {
        ...
        public double[] ReturnCentroid(out string errorMsg, string feature)
        {
            ...
            {
                shapeFileName = feature.Substring(0, feature.LastIndexOf('.'));
                shapeFileFeature = FeatureFromFile(out errorMsg, feature);
                coordinates = FindCentroid(out errorMsg, shapeFileFeature);
                coordinates = ConvertProjection(out errorMsg, coordinates);
            }
            else if (type.Contains(".json"))
            {
                shapeFileName = feature;
                shapeFileFeature = FeatureFromGeoJson(out errorMsg, feature);
                coordinates = FindCentroid(out errorMsg, shapeFileFeature);
            }
        ...
        private double[] ConvertProjection(out string errorMsg, double[] coord)
        {
            ...
            try
            {
                line = System.IO.File.ReadAllText(shapeFileName + ".prj");
            }
            catch
            {
        ...
        /// <param name="shapeFilename"></param>
        /// <returns></returns>
        private void ShapefileGridSearch(out string errorMsg, double[] coordinate, double cellWidth)
        {
            errorMsg = "";
            try
            {
        ...
        /// <param name="featureFile"></param>
        /// <returns></returns>
        private List<NetTopologySuite.Features.Feature> FeatureFromGeoJson(out string errorMsg, string geoJson)
        {
            errorMsg = "";
            List<NetTopologySuite.Features.Feature> featureCollection = new List<NetTopologySuite.Features.Feature>();
            try
            {
                ...
                string json = File.ReadAllText(geoJson);
                featureCollection.Add(geoReader.Read<Feature>(json));
            }
            catch (Exception ex)
            {
                errorMsg = "Error: Unable to load feature file. " + ex.Message;
        ...
        {
            errorMsg = "";
            if (shapeFileName.Contains(".json"))
            {
                FeatureGridSearch(out errorMsg, coordinate, cellWidth);
            }
            else
            {
                ShapefileGridSearch(out errorMsg, coordinate, cellWidth);
            }
        }
