...
namespace Radiation
{
    ...
    public class Daymet
    {
        ...
        public ITimeSeriesOutput GetData(out string errorMsg, ITimeSeriesOutput output, ITimeSeriesInput input)
        {
            ...
            daymetOutput = SetDataToOutput(out errorMsg, "Radiation", data, output, input);
            daymetOutput.Metadata["daymet_unit"] = (input.Units.Contains("imperial")) ? "in" : "mm";

            daymetOutput.Dataset = "Shortwave Radiation";
        ...
        private ITimeSeriesOutput TemporalAggregation(out string errorMsg, ITimeSeriesOutput output, ITimeSeriesInput input)
        {
            ...
            {
                case "monthly":
                    output.Metadata.Add("column_2", "shortwave");
                    output.Metadata.Add("column_2_units", "W/m^2");
        ...
        {
            errorMsg = "";
            string[] splitData = data.Split(new string[] { "year,yday,srad (W/m^2)" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        /// <param name="output"></param>
        /// <returns></returns>
        public static Dictionary<string, List<string>> MonthlyAverage(out string errorMsg, ITimeSeriesOutput output, ITimeSeriesInput input)
        {
            errorMsg = "";
            ...
            for (int i = 0; i < output.Data.Count; i++)
            {
                ...
                {
                    ssum = ssum / days;
                    output1.Data.Add(newDate.ToString("yyyy-MM-dd HH"), new List<string>() { ssum.ToString(input.DataValueFormat)});
                    newDate = iDate;
